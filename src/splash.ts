...
function updateSpeechBubbles(state: any) {
    ...
    const stateHandlers = {
        speaking: () => bubbles.forEach(bubble => {
            bubble.textContent = "Speaking...";
            bubble.classList.add("speaking");
        }),
        
        listening: () => bubbles.forEach(bubble => {
    ...
    if (state.matches({ PlayClue: "SayClue" }) || 
    ...
    else if (state.matches({ PlayClue: "GetAnswer" }) || 
...
dmActor.subscribe((state) => {
    ...
    if (difficulty && isValidDifficulty(difficulty)) cachedDifficulty = difficulty;
    ...
    if (cachedCategory && cachedDifficulty && 
    ...
    if (state.matches({ CorrectAnswer: "AskToRestart" })) {
      ...
      if (page4ClueNum) page4ClueNum.textContent = (currentClueIndex + 1).toString();
      ...
      if (page4DifficultyEl) page4DifficultyEl.textContent = cachedDifficulty ?? "N/A";
    }
    ...
    // ===== HANDLE LAST CLUE CASE =====
        ...
        const page4Transcript = page4.querySelector("#transcript") as HTMLElement;
        if (page4Transcript) {
          showTranscript(page4Transcript, `No more clues! The answer was ${currentItem?.name}.`);
        }
      ...
      const page4ClueText = page4.querySelector("#clue-text");
      if (page4ClueText) page4ClueText.textContent = `The answer was: ${currentItem?.name}`;
    ...
    // ===== ANALYZING STATE =====
    if (
      (state.matches("CorrectAnswer") && !state.matches({ CorrectAnswer: "AskToRestart" })) ||
      (state.matches("IncorrectAnswer") && !(state.matches({ IncorrectAnswer: "AskNextClue" }) || state.matches({ IncorrectAnswer: "HandleLastClue" })))
    ) {
    ...
    if (
        ...
        console.log("🔄 Restarting round - back to page 3");
      }
...
if (state.matches("GameOver")) {
    ...
    if (finalScoreEl) finalScoreEl.textContent = state.context.score?.toString() ?? "0";
  }
